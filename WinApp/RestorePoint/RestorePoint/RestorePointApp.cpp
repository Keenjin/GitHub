// RestorePoint.cpp : 定义控制台应用程序的入口点。
//
...
HRESULT BackupEnable(LPCWSTR wsDrive)
{
	HRESULT hr;
	...
	do
	{
		...
		hr = pRestoreCaller.Invoke1(L"Enable", &CComVariant(wsDrive));
		if (FAILED(hr))
			break;
...
HRESULT BackupRestore(UINT nId)
{
	...
	do
	{
		...
		if (FAILED(hr))
		{
			std::wcout << L"CoGetObj failed." << std::endl;
			break;
		}
		...
		if (FAILED(hr))
		{
			std::wcout << L"Restore failed." << std::endl;
			break;
		}
		hr = vtRet.ChangeType(VT_I4);
		if (FAILED(hr))
		{
			std::wcout << L"ChangeType failed." << std::endl;
			break;
		}
		...
		if (FAILED(hr))
		{
			std::wcout << L"end failed." << std::endl;
		}
		else
		{
			std::wcout << L"end succ." << std::endl;
		}
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	do {
		...
		/*hr = BackupCreate(L"asdf");
		if (FAILED(hr))
			break;*/

		CAtlArray<BACKUP_INFO> backups;
		hr = BackupGetList(backups);
		if (FAILED(hr))
			break;

		std::cout << "系统有如下还原点：\n";

		for (size_t i = 0; i < backups.GetCount(); i++)
		{
			std::wcout << L"还原点名称：" << backups[i].strName.GetString() << std::endl;
			std::wcout << L"还原点id：" << backups[i].ulId << std::endl;
		}

		if (backups.GetCount() == 0)
		{
			std::wcout << L"还原点个数为0" << std::endl;
		}
		else
		{
			std::wcout << L"请输入需要还原点id：" << std::endl;
			ULONG uid = 0;
			std::cin >> uid;
			BackupRestore(uid);
		}
